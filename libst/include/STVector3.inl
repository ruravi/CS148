...
inline float Modulus(const STVector3& v)
{
return sqrt(pow(v.x,2) + pow(v.y,2) + pow(v.z,2));

}

inline STVector3 Normalize(const STVector3& v)
{
    float modulus = sqrt(v.x*v.x + v.y*v.y + v.z*v.z);
    return STVector3((v.x)/modulus, (v.y)/modulus, (v.z)/modulus);
}
